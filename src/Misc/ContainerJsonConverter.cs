    ...
    public class ContainerJsonConverter : JsonConverter
    {
        public override bool CanConvert(Type objectType)
        {
            return objectType.GetInterfaces().Any(i =>
                i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IContainerJsonConverter));
        }

        public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
        {
            var methodGets = value.GetType().GetMethod("Gets", Type.EmptyTypes);
            if (methodGets == null)
                throw new MissingMethodException(value.GetType().FullName, "Gets");
            var valueArray = methodGets.Invoke(value, null);
